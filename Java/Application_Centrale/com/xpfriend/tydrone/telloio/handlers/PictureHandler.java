        ...
        if (id == 48) {
            ...
            if (!picture.chunkState[seqNum]) {
                ...
                if (picture.receivedSize >= picture.fileSize) {
                    ...
                    //picture.close(info);
                    this.monIHM.setVideo(new ImageIcon(this.getScaledImage(this.convertByteArrayToImage(picture.close(info)), 653, 379)));
                }
            }
        } else if (id == 100) {
            logd("completed");
